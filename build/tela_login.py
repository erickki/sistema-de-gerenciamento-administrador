...
def tela_login(page: ft.Page):
    ...
    def cadastrar_log(id_empresa_data, senha_data):
        banco_cadastros = os.path.join(os.path.dirname(__file__), '..', 'data', 'cadastros.db')
        banco_cadastros = os.path.abspath(banco_cadastros)
        conn = sqlite3.connect(banco_cadastros)
        cursor = conn.cursor()
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS tabela_cadastros_logs (
            ...
            cargo TEXT NOT NULL,
            data_hora_log TEXT NOT NULL,
            FOREIGN KEY (id_usuario) REFERENCES tabela_cadastros(id)
        );
        """)
        cursor.execute("""
        SELECT id, nome, email, id_empresa, cargo FROM tabela_cadastros
        WHERE id_empresa = ? AND senha = ?
        """, (id_empresa_data, senha_data))
        usuario = cursor.fetchone()
        if usuario:
            id_usuario, nome, email, id_empresa, cargo = usuario
            data_hora_log = datetime.now().strftime("%d/%m/%Y - %H:%M:%S")
            cursor.execute("""
            INSERT INTO tabela_cadastros_logs (id_usuario, nome, email, id_empresa, cargo, data_hora_log)
            VALUES (?, ?, ?, ?, ?, ?)
            """, (id_usuario, nome, email, id_empresa, cargo, data_hora_log))
            conn.commit()
        conn.close()


    def entrar_paineis_programa(e):
        if entrada1_login.content and entrada2_login.content:
            id_empresa_data = entrada1_login.content.value
            senha_data = entrada2_login.content.value
        else:
            return
        cargo_usuario = botao_logar_se(e)
        if cargo_usuario:
            if cargo_usuario == "Administrador":
                from build.tela_painel_administrador import tela_painel_administrador
                page.close(alerta1_login)
                time.sleep(0.1)
                page.clean()
                tela_painel_administrador(page)
                if id_empresa_data and senha_data:
                    cadastrar_log(id_empresa_data, senha_data)
                else:
                    None
                page.update()
            else:
                page.open(alerta3_login)
        else:
            page.open(alerta3_login)
